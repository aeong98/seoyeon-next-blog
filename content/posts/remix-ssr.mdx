---
title: Remix의 SSR, React 18 renderToPipeableStream을 위주로 살펴보기
...
PassThrough는 Node.js에서 스트리밍 데이터 처리를 위한 기본 스트림 클래스이다. (아무런 변형 없이 쓰기 쪽에서 읽기 쪽으로 데이터를 넘기는 스트림으로 파이프라인을 만들때 사용되는 클래스라고 한다.) PassThrough 스트림을 HTTP 응답 스트림으로 사용할 수 있도록 ReadableStream 으로 변환하고, 최종적으로 HTTP을 반환한다.
